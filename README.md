...
- **SplatViewer** *(v1.7.0)*: dedicated 3D Gaussian Splat renderer for splat-flavored `.ply` / `.splat` / `.ksplat` files, built on `@mkkellogg/gaussian-splats-3d`. Outlier-robust camera auto-fit (ignores stray "floater" splats a naive bounding-box fit would get wrecked by), shift+click custom orbit pivot, and keyboard fly-through: `W`/`S` forward-back, `A`/`D` strafe, `Q`/`E` turn, `R`/`F` up-down, `Z`/`C` orbit around the pivot, active while hovering the panel. `V` cycles up-axis orientation presets, since gaussian-splatting exports don't follow one universal up-axis convention.
